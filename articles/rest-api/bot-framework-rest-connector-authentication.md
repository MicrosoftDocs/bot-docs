...
### Step 1: Request an access token from the Entra ID account login service
...
### Step 2: Obtain the JWT token from the Entra ID account login service response
...
This example shows a response from the Entra ID account login service:
...
If the token is correctly formed, isn't expired, and was generated by the Entra ID account login service, the Bot Connector service will authorize the request. Additional checks are performed to ensure that the token belongs to the bot that sent the request.
...
> The JWT token that you obtain from the the Entra ID account login service is like a password and should be handled with
