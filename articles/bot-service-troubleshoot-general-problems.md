...
You can use the IP addresses, generated by the steps below, to  verify if there are any rules blocking the connection between those addresses. See section [Check firewall traces on failed connections](#check-firewall-traces-on-failed-connections).
