...
Use the following steps to check for any failed connections between the bot and the channel. If you find any failed connections, check whether there is a firewall rule that is blocking the connection.
...
1. Run `nslookup ivr-sr-bot.botapps.amat.com`. The DNS resolution is working, if he response to this command contains relevant information.
