...
For example, a bot that can check a user's recent emails, using the Microsoft Graph API, requires a user token from an **Identity Provider**, in this case **Microsoft Microsoft Entra ID**. At design time, the bot developer performs these two important steps:
