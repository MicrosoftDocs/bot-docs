...
Let's say you've developed the following services, and you want to create a bot that integrates with all of them.
...
The fastest way to create the HomeAutomation and Weather LUIS apps is to download the [homeautomation.json][HomeAutomationJSON] and [weather.json][WeatherJSON] files. Then go to [https://www.luis.ai/home](https://www.luis.ai/home) and sign in. Click on **My Apps** > **Import new app** and choose the homeautomation.json file. Name the new app `homeautomation`. Click on **My Apps** > **Import new app** and choose the weather.json file. Name this other new app `weather`.

## Create the QnA Maker service

Go to [https://qnamaker.ai](https://qnamaker.ai) and sign in. Select **Create new service** and create a new service named "SmartLightFAQ". Click the **Select file** button and upload the [sample TSV file][FAQ_TSV]. Then click **Create**. Once the service is created, click **Publish**.
...
Run the following command to initialize the Dispatch tool. Substitute your [LUIS authoring key](https://docs.microsoft.com/en-us/azure/cognitive-services/LUIS/luis-concept-keys) for `"YOUR-LUIS-AUTHORING-KEY"`.

```
dispatch init -name CombineWeatherAndLights -luisAuthoringKey "YOUR-LUIS-AUTHORING-KEY" -luisAuthoringRegion westus
```

For each of the LUIS apps that you created, get the LUIS app ID. Find the app under **My Apps** [https://www.luis.ai/home](https://www.luis.ai/home), click on the app name, and then click on **Settings** to see the Application ID. 
...
Now you can hook up the dispatcher app's intents to logic in your bot that routes messages to the original LUIS apps and QnAMaker service. The intent names used in the bot must match the intents in the dispatcher app.
...
Start with the code in the [LUIS Dispatch sample](https://github.com/Microsoft/botbuilder-dotnet/tree/master/samples/Microsoft.Bot.Samples.Ai.Luis.Dispatch). 
...
If there are user messages that are provided as examples in both the LUIS apps and the QnA maker services, the combined LUIS app that Dispatch generates won't perform well for those inputs. Remove similar or overlapping examples, and check your app's performance using the `-eval` option. 

For example, let's say that in the `homeautomation` LUIS app for homeautomation, requests like "turn my lights on" map to a "TurnOnLights" intent, but requests like "Why won't my lights turn on?" map to a "None" intent so that they can be passed on to QnA maker. When you combine the LUIS app and the QnA maker service using dispatch, you need to do one of the following: 
...
* If you don't remove the "None" intent from the original LUIS app, you need to <!-- either remove that intent from the Dispatch-generated LUIS app, or --> add logic in your bot to pass those messages that match that intent on to the QnA maker service.

<!-- links -->
<!-- TODO: UPDATE TO PERMANENT URL -->
[HomeAutomationJSON]: https://github.com/stevengum97/botbuilder-js/blob/m3-samples/samples/dispatch-es6/homeautomation.json
[WeatherJSON]: https://github.com/stevengum97/botbuilder-js/blob/m3-samples/samples/dispatch-es6/weather.json
[DispatchJSON]: https://github.com/stevengum97/botbuilder-js/blob/m3-samples/samples/dispatch-es6/dispatchSample.json
<!-- TODO: UPDATE TO current questions -->
[FAQ_TSV]: https://github.com/stevengum97/botbuilder-js/blob/m3-samples/samples/dispatch-es6/sampleKnowledgeBase.tsv

[DispatchTool]: https://github.com/Microsoft/botbuilder-tools/tree/master/Dispatch

[DispatchBotCS]: https://github.com/Microsoft/botbuilder-dotnet/tree/master/samples/Microsoft.Bot.Samples.Ai.Luis.Dispatch
<!-- TODO: UPDATE this path when sample is merged -->
[DispatchBotJs]: https://github.com/stevengum97/botbuilder-js/tree/m3-samples/samples/dispatch-es6
