...
This article explains the concepts behind declarative assets and bots that incorporate **Adaptive dialog** using the declarative approach.  Declarative adaptive dialogs consist of JSON based files that describe all of the attributes of an adaptive dialog including its [actions](bot-builder-concept-adaptive-dialog-actions.md) and [triggers](bot-builder-concept-adaptive-dialog-triggers.md). These declarative files are loaded using the resource manager to create the adaptive dialogs from them at run time.
...
Declarative files currently consist of `.dialog` files, a JSON based file that describe all of the attributes of an adaptive dialog and [.lg][lg-file-format] files that consist of LG templates that define the [language generation (LG)](bot-builder-concept-adaptive-dialog-generators.md) aspects of your bot.

### .dialog files

Adaptive dialog declarative files that have the .dialog extension contain the following elements:

- The `"$schema` keyword. The value is a URI pointing to a schema file. This schema file contains the schemas of all the component consumed by your bot, based on your nuget or npm package references. This schema file is what will enable [intellisense][intellisense] to work for all of your declarative elements. If it cannot find the referenced file it will not result in a warning or error and everything will work as expected, with the exception of intellisnse. For information on how to create this file see [Creating the schema file](bot-builder-dialogs-declarative.md#creating-the-schema-file) in the _Create a bot using declarative adaptive dialogs_ article. The name of the schema file can be any valid filename, but is typically named "app.schema".
- The `"$kind"` keyword specifies what kind of declarative asset is being referred to. The `"$kind"` keyword at the top level of the declarative file will always be `"$kind": "Microsoft.AdaptiveDialog"` for an adaptive dialog. In subsequent levels the `"$kind"` keyword will specify what type of trigger or action is being referenced. This keyword correlates with the `[JsonProperty("$kind")]` class attribute that is associated with every class in the Bot framework SDK designed to work using the declarative approach.
